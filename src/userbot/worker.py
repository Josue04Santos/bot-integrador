...
async def _process_one(item: QueueItem, bot: Bot) -> None:
    ...
    if batch_just_completed:
        await _notify_batch_complete(bot, item.batch_id, item.user_tg_id)
...
async def _notify_user(
    ...
    try:
        await bot.send_message(item.user_tg_id, text)
    except Exception:
        logger.exception("Falha ao notificar user", user_tg_id=item.user_tg_id)
